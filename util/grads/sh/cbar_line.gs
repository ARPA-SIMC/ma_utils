...
* Versione modificata J&E (cje), per scrivere la legenda anche con sfondo
* bianco. Prima di lanciare lo script, fissare il colore del testo con
* set string N (0)
*
...
placeit=0
***********************   USAGE   **************************************************
usage1="usage: cbar_line -x X -y Y -c color -m mark -l linestyle -t text -s size -p"
...
usage7="-s size     symbols size (Default 0.1)"
usage8="-t text     text for each line/symbol, must be double quoted"
usage9="-p          if set, user can click on graphic to set legend."
usage10=" "
usage11="If one keyword misses a value, uses default for the remaining"
usage12="To include zeros, zeros has to be double quoted"
usage13='Ex.: cbar_line -c 1 2 3 -m 1 2 3 -l 1 2 3 -s "0.1" "0.2" "0.1" -t "line1" "line2" "line3" -p'
  ...
  if (opt='x' | opt='y' | opt='X' | opt='Y')
   ...
   if (ttt='-' | ttt='')
    ...
    if (opt='x' | opt='X')
     ...
     say 'NOTE: if you have specified an X position < 1 (es. 0.xxx), '
     say '      you must not put the "0" (use .xxx)'
    endif
    if (opt='y' | opt='Y')
     yerr=1
     say '-y error: Y position missing!'
     say '          y=1 will be used.'
     say 'NOTE: if you have specified an Y position < 1 (es. 0.xxx), '
     say '      you must not put the "0" (use .xxx)'
    endif
   endif
...
while (p<maximum)
  p=p+1
*cje  if (p>k); cols.p=1; endif
  if (p>k); cols.p=0; endif
  if (p>l); lines.p=0; endif
  if (p>m); marks.p=0; endif
  if (p>s); sizem.p=0.1; endif
...
while (o<maximum)
  ...
  'draw mark 'marks.o' 'xm' 'y' 'sizem.o
  if(lines.o>0)
    'draw line 'xl' 'y' 'xr' 'y
  endif
  'set string 0 l 5'
