...
!
! Note:
! La chiave productDefinitionTemplateNumber viene forzata  0 (istantaneo)
! oppure 8 (eaborato), perdendo le informazioni accessorie (ens, chimica, etc.)
! Questo potrebbe essere reso opzionale.  
!
!                                         Versione 1.0.0, Enrico 24/04/2024
!--------------------------------------------------------------------------
...
! 2.5.1 Campo istantaneo
  IF (pdtn == 0 .OR. pdtn == 1 .OR. pdtn == 40 .OR. pdtn == 41) THEN  
    ...
    CALL grib_set(igout,"productDefinitionTemplateNumber",0)   ! table 4.0
...
! 2.5.2 Campo elaborato: trange="elaborato", reftime="inizio intervallo", forecastTime=0
    IF (tr_proc == "proc") THEN
      ...
      CALL grib_set(igout,"productDefinitionTemplateNumber",8) ! table 4.0
...
WRITE (*,*) "Uso: grib2_forc2ana.exe [-h] filein fileout [-isti / -istf]"
