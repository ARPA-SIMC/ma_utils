...
subroutine opfi(ioval,fich,form,status)
  ...
  if(form.eq.'f'.and.status.eq.'r')then
     open(unit=ioval,file=fich,err=800,status='REPLACE')
  endif
...
#if defined(IFORT)
  if(form.eq.'u'.and.status.eq.'o_convert_bigendian')then
     open(unit=ioval,file=fich,err=802, status='OLD',form='unformatted',convert='big_endian')
  endif
#endif
