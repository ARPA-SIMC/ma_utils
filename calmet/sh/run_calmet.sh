#!/bin/bash
###########################################################################
# Legacy script: this script can't be run anymore, but it may be a useful
# template for possible future Calmet applications
###########################################################################
#
# run_calmet.ksh
#
...
cal_root=$HOME/calmet
if [ ! -d $cal_root ] ; then
  echo "$cal_root not found"
  exit
fi
proj_dir=${cal_root}/progetti/${proj}            # files statici del progett
dat_dir=${cal_root}/dat/                         # files costanti calmet
run_root=${SCRATCH}/calmet/${proj}               # root esecuzione
...
zlay_lamaz_2003=/usr/share/ma_utils/LAMAZ_layers_20030402.grb
zlay_lamaz_2006=/usr/share/ma_utils/LAMAZ_layers_20060126.grb
zlay_lamaz_2012=/usr/share/ma_utils/LAMAZ_layers_20120606.grb
