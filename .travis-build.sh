...
$builddep -y fedora/SPECS/ma_utils.spec

if [[ $image =~ ^fedora: || $image =~ ^centos: ]]
then
    pkgname=ma_utils-$(git describe --abbrev=0 --tags --match='v*' | sed -e 's,^v,,g')
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp fedora/SPECS/ma_utils.spec ~/rpmbuild/SPECS/ma_utils.spec
    git archive --prefix=$pkgname/ --format=tar HEAD | gzip -c > ~/rpmbuild/SOURCES/$pkgname.tar.gz
    rpmbuild -ba ~/rpmbuild/SPECS/ma_utils.spec
